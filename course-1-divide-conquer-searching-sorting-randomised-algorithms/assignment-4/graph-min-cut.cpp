...
void pickEdgeRandomly(std::vector<vector<int>> &input, std::vector<int> &verticesToMerge)
{
    ...
    while (isVertexMergedAlready)
    {
        ...
        else
        {
            ...
            int randomEdgeTail = pickRandomVertex(rangeFrom,rangeTo);
            
            if ((input[randomEdgeHead])[randomEdgeTail] > (input[randomEdgeHead])[0])
            {
                verticesToMerge.push_back((input[randomEdgeHead])[0]);
                verticesToMerge.push_back((input[randomEdgeHead])[randomEdgeTail]);
                isVertexMergedAlready = false;
            }
            else if((input[randomEdgeHead])[randomEdgeTail] < (input[randomEdgeHead])[0])
            {
                verticesToMerge.push_back((input[randomEdgeHead])[randomEdgeTail]);
                verticesToMerge.push_back((input[randomEdgeHead])[0]);
                isVertexMergedAlready = false;
